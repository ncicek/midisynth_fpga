...
uint8_t handleNoteOff(struct voice voice_table[NUMBER_OF_VOICES], uint8_t voice_index, uint8_t midi_note) {
  voice_index = remove_note(voice_table, midi_note, voice_index);
...
void add_note(struct voice voice_table[NUMBER_OF_VOICES], uint8_t midi_note, voice_index){
  voice_table[voice_index].midi_note = midi_note;
  voice_table[voice_index].note_state = 1;
  return();
}

uint8_t remove_note(struct voice voice_table[NUMBER_OF_VOICES], uint8_t midi_note, voice_index){
